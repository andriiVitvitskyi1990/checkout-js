...
describe('MandateTextComponent',  () => {
    ...
    it('renders proper translation keys and values', async () => {
        ...
        // First li
        const firstItem = await screen.findByTestId('order-confirmation-mandate-mandateText-list-item-0');
        const firstItemKey = firstItem?.textContent?.split(':')[0];
        const firstItemValue =  firstItem?.textContent?.split(':')[1];

        // Second li
        const secondItem = await screen.findByTestId('order-confirmation-mandate-mandateText-list-item-1');
        const secondItemKey = secondItem?.textContent?.split(':')[0];
        const secondItemValue =  secondItem?.textContent?.split(':')[1];

        // Third li
        const thirdItem = await screen.findByTestId('order-confirmation-mandate-mandateText-list-item-2');
